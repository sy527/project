...
```{r, eval = FALSE}
library(tidyverse)
setwd("/Users/sunyoon/Desktop/myrepo/data")
...
Seems like buyers are clustered in certain areas. `fviz_nbclust()` takes a very long time, so I won't use that function to find the optimal number of clusters. Instead, I will do 4 clusters. 

```{r, results = F}

ca_data <- ca %>%
  select(longitude_b, latitude_b) %>%
  na.omit()
...
ca_data %>%
  na.omit() %>%
  qmplot(longitude_b, latitude_b, data = ., maptype = "toner-lite", size = I(0.75), alpha = I(0.75), color = as.factor(km_1$cluster))
